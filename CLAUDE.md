...
`main.py` (~2100 lines) is the hub: app setup, logging config, a request-timing middleware (logs every API call, flags >3s as SLOW), plus the sector-performance, news, movers, and weekly-review endpoints inline. Every other feature is a self-contained package/module exposing a FastAPI `APIRouter` that `main.py` registers. (Endpoints have been progressively extracted from the ~4700-line hub into routers — `scanners_router.py`, `journal_router.py`, `playbook_router.py`, `trading_analysis_router.py` (the 19 trade-log analytics routes), `backtest_router.py`, and `breadth_router.py`.)
