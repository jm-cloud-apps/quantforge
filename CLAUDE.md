...
`main.py` (~2600 lines) is the hub: app setup, logging config, a request-timing middleware (logs every API call, flags >3s as SLOW), plus the backtesting, breadth, sector-performance, news, and weekly-review endpoints inline. Every other feature is a self-contained package/module exposing a FastAPI `APIRouter` that `main.py` registers. (Endpoints have been progressively extracted from the hub into routers — `scanners_router.py`, `journal_router.py`, `playbook_router.py`, and `trading_analysis_router.py` (the 19 trade-log analytics routes); the breadth and backtesting clusters are the remaining candidates.)
