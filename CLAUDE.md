...
- `breadth/` — the market-context engine behind Trade Today / Market Monitor / Situational Awareness: `universe` (active US common-stock universe), `calculator` (Stockbee-style 4%/thrust/T2108 breadth), `situational` (exposure score + per-setup regime filter), `regime` / `regime_backtest` (forward-return validation of the filter), `calibration` (are the stance bands sized right for what they paid?), `sa_history` (persistent daily ledger), `index_trend`, and `verify` (independent pipeline recount). The HTTP endpoints (`/api/breadth/*`, memoized via `_breadth_cached` against a data fingerprint) live in `breadth_router.py`; `backtester/` is likewise fronted by `backtest_router.py`
  - **There is exactly one exposure score.** `situational.exposure_score()` is the single implementation; `regime.classify()` derives its tape label from it rather than tallying its own votes. It used to have its own ±1/±2 heuristic and the two surfaces drifted apart — Market Monitor read "Neutral / trade selectively" on a day Trade Today scored 35/100 Defensive. Never reintroduce a second scoring path: add factors to `situational.FACTORS` instead. Guarded by `tests/test_regime_calibration.py`
  - **Ledger buckets report episodes, not just days.** Regime days are autocorrelated — a constructive stretch is one event across fifteen rows. `regime_backtest` and `calibration` both attach `episodes` + a `reliability` verdict (`measured` ≥12 / `tentative` ≥5 / `insufficient`) to every bucket, matching `signal_scorecard`. Judge any bucket on its episode count
  - `calibration` is **read-only by design**: it reports what each band paid against `_EXPOSURE_WEIGHT` and suggests a weight shrunk toward the one in use by `episodes / (episodes + SHRINK_K)`. It never rewrites the live dial — refitting a sizing rule to one year of one market is the overfitting `analytics/edge_validation` exists to catch
- `scanners/` — pure scanner logic: `ep9m` ($9M episodic-pivot scanner), `reversal` (reversal setup), `stage_analysis` (Weinstein stages), `ma_reclaim` (200-day-MA reclaim), `parabolic` (Qullamaggie parabolic-short / over-extension fade), `rs_leaders` (the 6M/3M/1M prep scan). Their HTTP shells (thin cache + route) live in `scanners_router.py`, registered by `main.py`. All read the **shared breadth grouped-daily cache** (`breadth.cache`) rather than calling a provider — adding a scanner costs zero extra API calls. That cache is **unadjusted**, so anything reading multi-month price history needs a reverse-split guard (see `parabolic._is_probable_split`: a >90% one-day jump on *collapsed* volume is a split, not a move)
...
- `prep_router.py` + `scanners/rs_leaders.py` — the **Prep** page (weekend / evening research routine). `rs_leaders` ranks the liquid *common-stock* universe by return over 6M / 3M / 1M separately and marks names appearing in ≥2 windows as `confluence`; every candidate is triaged into `at_pivot` / `basing` / `extended` / `broken` so the list is actionable rather than just the most-extended names in the market. It restricts to `breadth.universe` (the raw grouped cache contains leveraged ETFs, which otherwise rank straight to the top of a return scan) and gates on **20-day average** dollar volume, not today's — a name that pumped today clears a single-day bar while being untradable all month. `/api/prep/session` persists each prep run (the market read at the time + the shortlist), which is what lets Discipline distinguish a planned trade from one found at 10am
...
- The sidebar (`components/Layout.jsx`) organizes pages by a trading **workflow**, not alphabetically: Overview → 1 · Market Context (Prep, Trade Today, Market Monitor, Theme Radar — Prep is first because it happens first, the night before / the weekend) → 2 · Find Setups (Setups Board, Stage Analysis, 200 MA Reclaim, Breakouts, $9M Scanner, Reversal Setup, Parabolic Short, Sector Scan, Earnings, Stock Analysis, Options Flow) → 3 · Plan & Execute (Rules, Playbook, Tools, Watchlist, AI Trader) → 4 · Review & Learn (Discipline, Review, Journal, Trading Analysis, Wealthsimple, Yearly Strongest) → Research & Validation (Edge Validation, Factor Model, Backtesting, Signal Lab, Bot Trader). Note "Trade Today" is the label for the `/situational-awareness` route.
